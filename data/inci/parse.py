...
# -*- coding: utf-8 -*-
"""
INCI Beauty Full Scraper - COMPLETE VERSION
Параллельный парсинг с ретраями + сбор ВСЕХ доступных данных
"""
...
BASE_URL = "https://incibeauty.com"  # Убраны лишние пробелы
...
# =============================================================================
# 🆕 ОБНОВЛЁННАЯ ФУНКЦИЯ: сбор ВСЕХ полей включая функции, описание, статистику
# =============================================================================
def scrape_ingredient_with_retries(
    ...
    """
    Парсит ингредиент с ретраями и собирает ПОЛНЫЕ данные:
    - functions (с названиями из INCI)
    - description (из секции "Namely")
    - prevalence (процент использования)
    - common_product_types (типы продуктов с процентами)
    """
    url = item['url']
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # === 🆕 ФУНКЦИИ (INCI) с извлечением названий ===
            functions = []
                ...
                if func_container:
                    ...
                    if func_list:
                        for li in func_list.find_all('li'):
                            ...
                            # Формат: "<i>Function Name :</i> Description"
                            if ':' in func_text:
            ...
            # === 🆕 ОПИСАНИЕ из секции "Namely" (div.section-zoom) ===
            description = ""
            namely_section = soup.find('div', class_='section-zoom')
            if namely_section:
                zoom_span = namely_section.find('span', class_='zoom')
                if zoom_span:
                    # Берём весь текст и убираем метку "Namely"
                    full_text = namely_section.get_text(strip=True)
                    description = full_text.replace('Namely', '', 1).strip()
                else:
                    description = namely_section.get_text(strip=True)
            
            # === 🆕 СТАТИСТИКА: prevalence и product types ===
            prevalence = "Данные недоступны"
            common_product_types = []
            
            progress_container = soup.find('div', class_='ingredient-progress-container')
            if progress_container:
                # Извлекаем общий процент
                prevalence_text = progress_container.find('strong')
                if prevalence_text:
                    prev_match = re.search(r'present in ([\d.]+)%', prevalence_text.get_text())
                    if prev_match:
                        prevalence = f"{prev_match.group(1)}% косметических средств"
                
                # Извлекаем типы продуктов из progress-bar
                for progress_bar in progress_container.find_all('div', class_='progress-bar'):
                    bar_text = progress_bar.get_text(strip=True)
                    # Формат: "Product Name (XX.XX%)"
                    match = re.match(r'(.+?)\s*\(([\d.]+)%\)', bar_text)
                    if match:
                        product_name = match.group(1).strip()
                        product_percent = match.group(2).strip()
                        common_product_types.append(f"{product_name} ({product_percent}%)")
            ...
            # === Формируем name и note ===
            name = inci_name_parsed.title() if inci_name_parsed else ""
            
            # Если описание пустое — генерируем fallback
            if not description:
            ...
            if origin:
                note_parts.append(f"Происхождение: {origin}.")
            
            note = " ".join(note_parts) if note_parts else "Данных недостаточно для оценки."
            
            # === Итоговый объект с ПОЛНЫМИ данными ===
            return {
                "name": name,
                ...
                "common_product_types": common_product_types if common_product_types else [],
                "note": note
            }, True
    ...
    if failed:
        ...
        for i, item in enumerate(failed, 1):
            ...
            if success and data:
                ...
                print("✗")
                failed_final.append({**item, 'error': 'max_retries_exceeded'})
...
def main():
    """Точка входа"""
    print("🧪 INCI Beauty Scraper - COMPLETE")
