...
def call_recommendation_api(query: str) -> Tuple[str, List[str]]:
    """
    Обёртка для H1: вызывает /rag/ask с промптом для рекомендаций товаров.
    Возвращает: (текст ответа, список retrieved article)
    """
    return call_rag_ask(query, system_prompt="system_common_prompt")


def call_analysis_api(product_info: str) -> Dict[str, str]:
    """
    Обёртка для H2: вызывает /rag/ask с промптом для классификации ингредиентов.
    Возвращает: словарь {ингредиент: категория}
    """
    # Формируем запрос с явной инструкцией вернуть JSON
    query_text = (
        f"Проанализируй следующие ингредиенты косметического средства и верни их классификацию "
        f"строго в формате JSON {{'название_ингредиента': 'категория'}}. "
        f"Категории: safe, caution, avoid, neutral. "
        f"Список ингредиентов: {product_info}"
    )
    
    rag_answer, _ = call_rag_ask(query_text, system_prompt="system_ingredients_prompt")
    ...
    try:
        ...
        if isinstance(parsed, dict):
            # Нормализуем ключи и значения к lower() для надёжного сравнения с GT
    ...
    for line in rag_answer.split('\n'):
        ...
        if ':' in line and not line.startswith(('{', '}', '[', ']')):
            ...
            if len(parts) == 2 and parts[0].strip():
                ...
                # Убираем лишние кавычки и запятые
                key = key.strip('"\' ,')
                val = val.strip('"\' ,')
                if key and val:
                    result[key] = val
