...
# 2. Строим пути относительно корня
...
judge_client = OpenAI(
    ...
    base_url=settings.model_api or "https://routerai.ru/api/v1",
    timeout=120,
    max_retries=3
)
...
def call_llm_judge(claim: str, reference_text: str, seed: int = 42, use_cache: bool = True) -> str:
    """
    Отправляет утверждение LLM-судье через OpenAI-клиент.
...
UNVERIFIED - если в Reference нет информации для проверки"""
    ...
    try:
        ...
        status = e.http_status if hasattr(e, 'http_status') else 'N/A'
...
def call_rag_ask_api(query: str) -> Tuple[str, List[str]]:
    """
    Вызывает RAG-эндпоинт: POST /rag/ask
    Возвращает: (текст ответа, список retrieved SKU/ID)
    
    Ожидает ответ по схеме ModelResponse:
    {
        "response": str,           # ← текст ответа
        "tokens": Dict,            # статистика токенов
        "source_nodes": List[Dict], # ← источники с метаданными
        "metadata": Dict           # доп. метаданные
    }
    """
    ...
    try:
        ...
        # По схеме: поле называется "response" (не "answer"!)
        rag_answer = data.get("response", "")
        
        # === 2. Извлекаем SKU/ID из source_nodes ===
        retrieved_ids = []
        ...
        if isinstance(source_nodes, list):
            for node in source_nodes:
                if isinstance(node, dict):
                    # Приоритет 1: поле article/sku в корне ноды (редко, но бывает)
                    item_id = node.get('article') or node.get('sku')
                    
                    # Приоритет 2: поле article/sku в metadata (основной случай для Chroma)
                    if not item_id:
                    ...
                    # Приоритет 3: если есть только node_id, пробуем найти article в тексте
                    if not item_id and node.get('text'):
                        extracted = extract_skus_from_text(node.get('text', ''))
                        if extracted:
                            item_id = extracted[0]
        ...
        # Fallback: парсим SKU из основного ответа, если source_nodes пустые
        ...
        logger.error(f"HTTP Error: {e} | Response: {e.response.text[:300] if e.response else 'N/A'}")
        return "", []
    except Exception as e:
...
def call_recommendation_api(query: str) -> Tuple[str, List[str]]:
    """Обёртка для H1: вызывает /rag/ask для рекомендаций"""
    return call_rag_ask_api(query)


def call_analysis_api(product_info: str) -> Dict[str, str]:
    """
    Обёртка для H2: вызывает /rag/ask для анализа ингредиентов.
    Возвращает словарь {ингредиент: категория}.
    """
    # Формируем запрос с явной инструкцией вернуть JSON
    query_text = (
        f"Проанализируй ингредиенты и верни классификацию строго в формате JSON "
        f"{{'название_ингредиента': 'категория'}}. Список: {product_info}"
    )
    
    rag_answer, _ = call_rag_ask_api(query_text)
    
    # Парсинг JSON из ответа LLM
    try:
        clean = rag_answer.replace("```json", "").replace("```", "").strip()
        parsed = json.loads(clean)
        if isinstance(parsed, dict):
            # Нормализуем ключи и значения к lower() для надёжного сравнения
            return {str(k).lower().strip(): str(v).lower().strip() for k, v in parsed.items()}
    except json.JSONDecodeError:
        logger.warning(f"Failed to parse JSON from analysis API: {rag_answer[:150]}...")
    
    # Fallback: парсим ключ-значение из текста, если JSON не валидный
    result = {}
...
# ==============================================================================
# 5. МЕТРИКИ (Без изменений, они универсальны)
# ==============================================================================
...
def calculate_accuracy_for_h2(predictions: Dict[str, str], ground_truth: Dict[str, str]) -> float:
    ...
    for ingredient, true_category in ground_truth.items():
        pred_category = predictions.get(ingredient.lower().strip())
...
def run_h1_experiment(limit_queries: int = None, seed: int = 42, use_judge: bool = True):
    """
...
def run_h2_experiment(limit_products: int = None, seed: int = 42):
    ...
    for i, product_data in enumerate(products):
        ...
        predictions = call_analysis_api(", ".join(inci_list))
