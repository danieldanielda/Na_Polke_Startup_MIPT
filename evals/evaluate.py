...
def normalize_ingredient_key(key: str) -> str:
    """
    Приводит ключ ингредиента к виду, совместимому с predictions от агента.
    1. Пробует прямой маппинг рус→лат
    2. Если нет — транслитерирует и приводит к lower
    """
    ...
    # 2. Простая транслитерация для кириллицы (базовая)
    if any(ord(c) > 127 for c in key_clean):  # есть кириллица
        # Очень упрощённая транслитерация (можно расширить)
